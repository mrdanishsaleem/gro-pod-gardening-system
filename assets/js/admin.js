jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#add_vegetable', function(e) {
        ...
        // Get next ID
        var nextId = 1;
        $('#vegetables-list tr').each(function() {
        ...
        actionKeys.forEach(function(action) {
            ...
            actionImagesHtml += '<div class="tpgs-action-input">' +
                ...
                '<input type="text" name="tpgs_vegetables[' + nextId + '][action_images][' + action + ']" value="' + (actionImages[action] || '') + '" class="regular-text">' +
                '</div>';
        });

        // Create new row
        var newRow = '<tr>' +
            '<td><input type="hidden" name="tpgs_vegetables[' + nextId + '][id]" value="' + nextId + '">' +
            '<input type="text" name="tpgs_vegetables[' + nextId + '][name]" value="' + name + '" class="regular-text"></td>' +
            '<td><input type="text" name="tpgs_vegetables[' + nextId + '][icon]" value="' + icon + '" class="regular-text"></td>' +
            '<td><input type="text" name="tpgs_vegetables[' + nextId + '][main_image]" value="' + mainImage + '" class="regular-text"></td>' +
            '<td>' + actionImagesHtml + '</td>' +
            '<td><input type="number" name="tpgs_vegetables[' + nextId + '][growth_duration]" value="' + duration + '" min="1" class="small-text"></td>' +
